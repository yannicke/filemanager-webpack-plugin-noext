        ...
        fs.lstat(command.source, function (sErr, sStats) {
          ...
          fs.lstat(command.destination, function (dErr, dStats) {
            if (sStats.isFile()) {
              ...
              if (pathInfo.ext === '' && command.toType !== 'file') {
    ...
    value: function parseFileOptions(options) {
      ...
      Object.keys(options).forEach(function (actionType) {
        ...
        actionOptions.forEach(function (actionItem) {
          switch (actionType) {
            case 'copy':
              actionParams = Object.assign({ source: _this2.replaceHash(actionItem.source) }, actionItem.destination && { destination: actionItem.destination }, actionItem.toType && { toType: actionItem.toType });
